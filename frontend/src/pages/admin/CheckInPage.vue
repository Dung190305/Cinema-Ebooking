            ...
            <div v-if="isCameraActive" class="pointer-events-none absolute inset-0">
              ...
              <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                <div class="h-48 w-48 rounded-2xl border-2 border-white/80 shadow-2xl">
                  <div class="absolute -left-1 -top-1 h-6 w-6 border-l-4 border-t-4 border-accent rounded-tl-xl" />
                  <div class="absolute -right-1 -top-1 h-6 w-6 border-r-4 border-t-4 border-accent rounded-tr-xl" />
                  <div class="absolute -bottom-1 -left-1 h-6 w-6 border-b-4 border-l-4 border-accent rounded-bl-xl" />
                  <div class="absolute -bottom-1 -right-1 h-6 w-6 border-b-4 border-r-4 border-accent rounded-br-xl" />
                </div>
              </div>
            </div>
      ...
      <div class="flex flex-col gap-4">
        ...
        >
          ...
          <div class="mt-4 rounded-lg bg-slate-50 p-4">
            <p class="mb-2 text-xs font-medium text-slate-500">Ghế đã đặt ({{ booking.seats?.length ?? 0 }})</p>
          ...
          <div v-if="booking.combos?.length" class="mt-3 rounded-lg bg-slate-50 p-4">
            <p class="mb-2 text-xs font-medium text-slate-500">Combo ({{ booking.combos?.length }})</p>
            <div class="space-y-1 text-xs text-slate-600">
              <div v-for="combo in booking.combos" :key="combo.comboId" class="flex justify-between">
        ...
        <div v-if="booking && !isLoading">
          ...
          <div
            v-else
            class="rounded-lg bg-slate-50 p-3 text-center text-sm text-slate-400"
          >
            {{ booking.status === 'PENDING' ? 'Booking chưa thanh toán, không thể check-in' : 'Booking đã bị hủy' }}
          </div>
        </div>

        <!-- Check-in results -->
        <div
          v-if="checkInResults.length"
          class="rounded-xl border border-slate-200 bg-white p-6"
        >
...
import { defineComponent, h, onUnmounted, ref, computed } from 'vue'
