    ...
    <div class="w-full max-w-360 mx-auto px-4 lg:px-8 xl:px-16 py-8">
        ...
        <div class="flex justify-center mb-10">
            <div ref="tabsContainer"
                ...
                <button ref="nowShowingTabRef" @click="setStatus('NOW_SHOWING')"
                    class="relative z-10 flex items-center gap-2 px-6 py-2.5 text-sm font-medium transition-colors duration-300 rounded-full"
                    :class="currentStatus === 'NOW_SHOWING'
                        ? 'text-text-on-accent'
                        : 'text-text-secondary hover:text-text-primary'
                        ">
                    <PlayIcon class="w-4 h-4 transition-transform"
                        :class="{ 'scale-110': currentStatus === 'NOW_SHOWING' }" />
                    <span>Đang chiếu</span>
                </button>
                <button ref="comingSoonTabRef" @click="setStatus('COMING_SOON')"
                    class="relative z-10 flex items-center gap-2 px-6 py-2.5 text-sm font-medium transition-colors duration-300 rounded-full"
                    :class="currentStatus === 'COMING_SOON'
                        ? 'text-text-on-accent'
                        : 'text-text-secondary hover:text-text-primary'
                        ">
                    <ClockIcon class="w-4 h-4 transition-transform"
                        :class="{ 'scale-110': currentStatus === 'COMING_SOON' }" />
                    <span>Sắp chiếu</span>
                </button>

                <!-- Sliding Indicator -->
                <div class="indicator absolute top-1.5 bottom-1.5 rounded-full bg-accent shadow-lg shadow-accent/40 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
                    :style="indicatorStyle" />
            </div>
        </div>

        <!-- Skeleton Loading Grid (3 hàng x 4 cột) -->
...
function getTabRect(tabEl: HTMLElement | null, containerEl: HTMLElement | null) {
    if (!tabEl || !containerEl) return { left: 0, width: 0 }
